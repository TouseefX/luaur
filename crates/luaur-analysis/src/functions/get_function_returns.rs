...
pub unsafe fn get_function_returns(l: *mut lua_State) -> core::ffi::c_int {
    ...
    if argument_count != 1 {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.returns: expected 1 arguments, but got {}",
                argument_count
            ),
        );
    }
    ...
    if tfft.is_null() {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.returns: expected self to be a function, but got {} instead",
                get_tag(l, self_ty)
            ),
        );
    }

    push_type_pack(l, (*tfft).ret_types);
    1
}
