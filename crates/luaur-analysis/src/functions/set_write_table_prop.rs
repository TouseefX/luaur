...
pub unsafe fn set_write_table_prop(l: *mut lua_State) -> i32 {
    ...
    if argument_count < 2 || argument_count > 3 {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.setwriteproperty: expected 2-3 arguments, but got {}",
                argument_count
            ),
        );
    }
    ...
    if tftt.is_null() {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.setwriteproperty: expected self to be a table, but got {} instead",
                get_tag(l, self_ty)
            ),
        );
    }
    ...
    if luaur_common::FFlag::LuauTypeFunctionSupportsFrozen.get() && (*self_ty).frozen {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.setwriteproperty: cannot be called to mutate a frozen type, use `types.copy` to make a copy"
            ),
        );
    }
    ...
    if tfst.is_null() {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.setwriteproperty: expected to be given a singleton type, but got {} instead",
                get_tag(l, key)
            ),
        );
    }
    ...
    if tfsst.is_none() {
        lua_l_error_l(
            ...
            c"%s".as_ptr(),
            core::format_args!(
                "type.setwriteproperty: expected to be given a string singleton type, but got {} instead",
                get_tag(l, key)
            ),
        );
    }
    ...
    if let Some(prop) = (*tftt).props.get_mut(&key_name) {
        ...
        (*tftt)
            .props
            .insert(key_name, TypeFunctionProperty::writeonly(value));
    }
