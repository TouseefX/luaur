...
//!   - translates_to -> rust_item Inference::Inference

// Dead duplicate skeleton node: the canonical ctor is implemented elsewhere.
pub fn inference_inference() {
    unreachable!("canonical Inference::inference_type_id_refinement_id (C++ `explicit Inference(TypeId, RefinementId)`) lives in crates/luau-analysis/src/records/inference.rs; this skeleton node is unused");
}
