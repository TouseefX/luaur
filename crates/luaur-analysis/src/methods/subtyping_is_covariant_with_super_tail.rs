...
impl Subtyping {
    pub fn is_covariant_with_super_tail(
        &mut self,
        env: &mut SubtypingEnvironment,
        output_result: &mut SubtypingResult,
        sub_tp: TypePackId,
        sub_head_start_index: usize,
        sub_head: &Vec<TypeId>,
        sub_tail: Option<TypePackId>,
        super_tp: TypePackId,
        super_tail: TypePackId,
        scope: *mut Scope,
    ) -> EarlyExit {
        let _ = super_tp;

        if let Some(vt) = unsafe { get_type_pack_id::<VariadicTypePack>(super_tail).as_ref() } {
            for i in sub_head_start_index..sub_head.len() {
                let mut next = self
            ...
            EarlyExit::No
        } else if !unsafe { get_type_pack_id::<GenericTypePack>(super_tail).is_null() } {
            ...
            EarlyExit::Yes
        } else if !unsafe { get_type_pack_id::<ErrorTypePack>(super_tail).is_null() } {
            let mut r = SubtypingResult {
                is_subtype: true,
                ..Default::default()
            };
