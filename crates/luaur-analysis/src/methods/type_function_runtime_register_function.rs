...
impl TypeFunctionRuntime {
    pub unsafe fn register_function(
        ...
        // catch (CompileError& e) { return TypeFunctionError{Location{}, FailedToCompile{name.value, e.what()}}; }
            ...
            if let Some(e) = payload.downcast_ref::<CompileError>() {
                ...
                return Some(
                    TypeFunctionError::type_function_error_location_type_function_error_data(
                        Location::default(),
                        TypeFunctionErrorData::V3(FailedToCompile::new(name_str.clone(), what)),
                    ),
                );
            }
        ...
        if let Some(error) = check_result_for_error(l, &name_str, load_result) {
        ...
        if let Some(error) = check_result_for_error(l, &name_str, resume_result) {
            popper.LuauTempThreadPopper();
            return Some(error);
        }

        // if (!lua_isfunction(L, -1)) { lua_pop(L, 1); return TypeFunctionError{Location{}, TypeFunctionMissing{name.value}}; }
        if !luaur_vm::lua_isfunction!(l_vm, -1) {
            ...
            return Some(
                TypeFunctionError::type_function_error_location_type_function_error_data(
                    Location::default(),
                    TypeFunctionErrorData::V4(TypeFunctionMissing::new(name_str)),
                ),
            );
        }
