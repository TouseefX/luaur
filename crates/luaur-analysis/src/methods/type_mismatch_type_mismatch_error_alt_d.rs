...
impl TypeMismatch {
    // C++ `TypeMismatch::TypeMismatch(TypeId wantedType, TypeId givenType, std::string reason, std::optional<TypeError> error)`
    // (Error.cpp:1039): wraps the optional error into a shared pointer (`Arc`) when present.
    pub fn from_wanted_given_reason_error(
        wanted_type: TypeId,
        given_type: TypeId,
