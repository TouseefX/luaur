...
//! line editor replaced by `rustyline`. Mirrors
//! `luau-repl-cli/src/functions/run_repl_impl.rs`, adapted to the cli-test
//! module layout: `lua_State` under `records::lua_state`, `run_code` takes a
//! `&String`, and incomplete-chunk detection compiles through the C-ABI
//! `luau_compile` (cli-test's `copts()` returns the C-ABI `LuaCompileOptions`).
//!
...
// the "<eof>" suffix that marks an incomplete statement. `luau_compile` returns
...
fn is_incomplete_chunk(source: &str) -> bool {
    ...
    let bytes = unsafe { core::slice::from_raw_parts(bytecode as *const u8, bytecode_size) };
    ...
    let incomplete = if bytes.first() == Some(&0) {
        let message = &bytes[1..];
        message.ends_with(b"<eof>")
    } else {
        false
    };
