...
//! Node: `cxx:Test:Luau.CLI.Test:tests/RequireByString.test.cpp:1220:require_by_string_require_export_class`
...
//! - outgoing:
//!   - type_ref -> record ReplWithPathFixture (tests/RequireByString.test.cpp)
...
//!   - translates_to -> rust_item require_by_string_require_export_class
...
fn require_by_string_require_export_class() {
    ...
    luaur_common::FFlag::DebugLuauUserDefinedClasses.push_test_override(true);
    luaur_common::FFlag::DebugLuauUserDefinedClassesRuntime.push_test_override(true);
    struct Sff;
    impl Drop for Sff {
        fn drop(&mut self) {
            luaur_common::FFlag::DebugLuauUserDefinedClassesRuntime.pop_test_override();
            luaur_common::FFlag::DebugLuauUserDefinedClasses.pop_test_override();
