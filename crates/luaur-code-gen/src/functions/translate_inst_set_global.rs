...
pub fn translate_inst_set_global(build: &mut IrBuilder, pc: *const Instruction, pcpos: i32) {
    let insn = unsafe { *pc };
    ...
    build.inst_ir_cmd_ir_op_ir_op(IrCmd::CHECK_READONLY, env, fallback);
    ...
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::STORE_TVALUE, addr_slot_el, tva, offset_val);

    let undef = build.undef();
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::BARRIER_TABLE_FORWARD, env, reg_ra, undef);
    ...
    let reg_ra = build.vm_reg(ra);
    let aux_op = build.vm_const(aux);
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::FALLBACK_SETGLOBAL, pcpos_op, reg_ra, aux_op);
