...
pub fn translate_inst_set_table_n(build: &mut IrBuilder, pc: *const Instruction, pcpos: i32) {
    let insn = unsafe { *pc };
    ...
    if is_userdata_bytecode_type(bc_types.a) {
        ...
        build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::SET_TABLE, reg_ra, reg_rb, const_uint_c_plus_1);
        return;
    }
    ...
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CHECK_ARRAY_SIZE, vb, const_int_c, fallback);
    ...
    let arr_el = build.inst_ir_cmd_ir_op_ir_op(IrCmd::GET_ARR_ADDR, vb, const_int_zero);

    let reg_ra = build.vm_reg(ra);
    let tva = build.inst_ir_cmd_ir_op(IrCmd::LOAD_TVALUE, reg_ra);
    let const_int_c_times_sizeof_tvalue = build.const_int(
        (c as usize * core::mem::size_of::<luaur_vm::type_aliases::t_value::TValue>()) as i32,
    );
    ...
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::BARRIER_TABLE_FORWARD, vb, reg_ra, undef);
