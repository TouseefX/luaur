...
//
// Phase 2: ported verbatim from mlua's `tests/buffer.rs` (import-swap only:
// `mlua` -> `luaur_rt`). Exercises the Luau `buffer` runtime type, the
// `Buffer` handle, `Lua::create_buffer{,_with_capacity}`, and the
// `io::Read`/`io::Write`/`io::Seek` cursor adapter.

use std::io::{Read, Seek, SeekFrom, Write};

use luaur_rt::{Lua, Result, Value};
