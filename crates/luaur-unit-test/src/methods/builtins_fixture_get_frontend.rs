...
//! Node: `cxx:Method:Luau.UnitTest:tests/Fixture.cpp:778:builtins_fixture_get_frontend`
...
//!   - type_ref <- method Fixture::getFrontend (tests/Fixture.cpp)
//! - outgoing:
//!   - type_ref -> record Frontend (Analysis/include/Luau/Frontend.h)
//!   - type_ref -> method Fixture::getFrontend (tests/Fixture.cpp)
//!   - calls -> method Fixture::getFrontend (tests/Fixture.cpp)
...
//!   - translates_to -> rust_item BuiltinsFixture::getFrontend

// Dead duplicate skeleton node: the canonical method is implemented elsewhere.
pub fn builtins_fixture_get_frontend() {
    unreachable!("canonical BuiltinsFixture::get_frontend lives in crates/luau-unit-test/src/records/builtins_fixture.rs; this skeleton node is unused");
}
