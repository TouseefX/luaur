...
impl crate::records::bytecode_inliner_fixture::BytecodeInlinerFixture {
    pub fn extract_string_table(&self, bcb: &BytecodeBuilder) -> Vec<alloc::string::String> {
        let bytecode = bcb.get_bytecode();
        let data = bytecode.as_ptr() as *const core::ffi::c_char;
        let mut offset: usize = 2; // skip versions
        let mut result: Vec<alloc::string::String> = Vec::new();
        ...
        for _ in 0..strings_count {
            ...
            let s = alloc::string::String::from_utf8_lossy(str_bytes).into_owned();
            offset += str_len;
            result.push(s);
        }
