...
fn ir_builder_dse_vm_exit_sync_multiple_exit_registers() {
    ...
    let mut fix = IrBuilderFixture::new();
    {
        ...
        let add = b.inst_ir_cmd_ir_op_ir_op(IrCmd::ADD_NUM, load, one);
        let r1 = b.vm_reg(1);
        let tnumber = b.const_tag(TNUMBER);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_TAG, r1, tnumber);
        let r1 = b.vm_reg(1);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_DOUBLE, r1, add);
        let r2 = b.vm_reg(2);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_DOUBLE, r2, load);
        let r2 = b.vm_reg(2);
        let tnumber = b.const_tag(TNUMBER);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_TAG, r2, tnumber);
        let r3 = b.vm_reg(3);
        let tag = b.inst_ir_cmd_ir_op(IrCmd::LOAD_TAG, r3);
        let tnumber = b.const_tag(TNUMBER);
