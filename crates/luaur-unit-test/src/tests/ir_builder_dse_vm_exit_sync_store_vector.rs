...
fn ir_builder_dse_vm_exit_sync_store_vector() {
    ...
    let mut fix = IrBuilderFixture::new();
    {
        ...
        let x = b.inst_ir_cmd_ir_op_ir_op(IrCmd::LOAD_FLOAT, r0, zero);
        let r0 = b.vm_reg(0);
        let four = b.const_int(4);
        let y = b.inst_ir_cmd_ir_op_ir_op(IrCmd::LOAD_FLOAT, r0, four);
        let r0 = b.vm_reg(0);
        let eight = b.const_int(8);
        let z = b.inst_ir_cmd_ir_op_ir_op(IrCmd::LOAD_FLOAT, r0, eight);
        let r1 = b.vm_reg(1);
        b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op(IrCmd::STORE_VECTOR, r1, x, y, z);
        let r1 = b.vm_reg(1);
        let tvector = b.const_tag(TVECTOR);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_TAG, r1, tvector);
        let r2 = b.vm_reg(2);
        let tag = b.inst_ir_cmd_ir_op(IrCmd::LOAD_TAG, r2);
