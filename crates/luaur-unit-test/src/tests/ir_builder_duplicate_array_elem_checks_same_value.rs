...
fn ir_builder_duplicate_array_elem_checks_same_value() {
    ...
    let mut fix = IrBuilderFixture::new();
    {
        ...
        let r2 = b.vm_reg(2);
        let index = b.inst_ir_cmd_ir_op(IrCmd::LOAD_DOUBLE, r2);
        let valid_index = b.inst_ir_cmd_ir_op_ir_op(IrCmd::TRY_NUM_TO_INDEX, index, fallback);
        let one = b.const_int(1);
        let valid_offset = b.inst_ir_cmd_ir_op_ir_op(IrCmd::SUB_INT, valid_index, one);
        b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CHECK_ARRAY_SIZE, table1, valid_offset, fallback);
        ...
        let valid_index2 = b.inst_ir_cmd_ir_op_ir_op(IrCmd::TRY_NUM_TO_INDEX, index, fallback);
        let one = b.const_int(1);
        let valid_offset2 = b.inst_ir_cmd_ir_op_ir_op(IrCmd::SUB_INT, valid_index2, one);
        b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CHECK_ARRAY_SIZE, table1, valid_offset2, fallback);
        let zero = b.const_int(0);
        let elem2 = b.inst_ir_cmd_ir_op_ir_op(IrCmd::GET_ARR_ADDR, table1, zero);
        let zero = b.const_int(0);
