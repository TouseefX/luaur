...
fn ir_builder_full_store_has_to_be_observable_from_fallbacks_3() {
    ...
    let mut fix = IrBuilderFixture::new();
    {
        ...
        b.begin_block(entry);
        let r1 = b.vm_reg(1);
        let tag = b.inst_ir_cmd_ir_op(IrCmd::LOAD_TAG, r1);
        let tfunction = b.const_tag(TFUNCTION);
        b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CHECK_TAG, tag, tfunction, fallback);
        let constant = b.vm_const(10);
        let pointer = b.inst_ir_cmd_ir_op(IrCmd::LOAD_POINTER, constant);
        let r1 = b.vm_reg(1);
        b.inst_ir_cmd_ir_op_ir_op(IrCmd::STORE_POINTER, r1, pointer);
