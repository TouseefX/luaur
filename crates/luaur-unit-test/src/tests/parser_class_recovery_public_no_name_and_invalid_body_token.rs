...
fn parser_class_recovery_public_no_name_and_invalid_body_token() {
    ...
    assert!(!cls.is_null());
    let cls = unsafe { &*cls };
    assert!(unsafe { (*cls.name).name }.operator_eq_c_char(c"Foo".as_ptr()));

    assert_eq!(cls.members.size, 3);

    let m1 = unsafe { &*cls.members.data.add(0) }.get_if::<AstClassProperty>();
    assert!(m1.is_some());
    assert!(m1.unwrap().name.operator_eq_c_char(c"propone".as_ptr()));
    ...
    assert!(m2
        .unwrap()
        .function_name
        .operator_eq_c_char(c"methodone".as_ptr()));
    ...
    assert!(m3
        .unwrap()
        .function_name
        .operator_eq_c_char(c"methodtwo".as_ptr()));
}
