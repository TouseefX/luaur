...
fn subtyping_fn_arguments() {
    ...
    let super_ty =
        fixture.fn_item_initializer_list_type_id_initializer_list_type_id(vec![string_ty], vec![]);
    ...
    let expected = SubtypingReasoning::new(
        sub_path.args().index(0).build(),
        super_path.args().index(0).build(),
