...
//! Node: `cxx:Test:Luau.UnitTest:tests/TypeInfer.typePacks.test.cpp:1028:type_infer_type_packs_detect_cyclic_typepacks_2`
...
//!   - translates_to -> rust_item type_infer_type_packs_detect_cyclic_typepacks_2
...
fn type_infer_type_packs_detect_cyclic_typepacks_2() {
    ...
    assert_eq!("Unknown type 't0'", to_string_type_error(&result.errors[0]));
    let exits = unsafe { get_type_error::<FunctionExitsWithoutReturning>(&result.errors[1]) };
    assert!(
        !exits.is_null(),
        "expected FunctionExitsWithoutReturning, got {:?}",
        result.errors[1]
    );
}
